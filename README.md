...
- **每次 push**：重新打包 fpk，将 git 标签 `v{version}` 移到最新提交，并覆盖该 Release 的安装包
- **升版**：修改 `package.json` 与 `fpk/manifest` 中的 `version` 后 push，会新建 `v新版本` 的 Release；旧版本 Release 仍保留

也可在 Actions 页手动 **Run workflow** 触发一次构建。

**Fork 仓库 Actions 不跑？** GitHub 对 fork **默认关闭** Actions，需先开启：

1. 打开 `https://github.com/<你的用户名>/ktpwarp-server/settings/actions`
2. **Actions permissions** → 选 **Allow all actions and reusable workflows**
3. **Workflow permissions** → 选 **Read and write permissions**（否则无法创建 Release、推送 tag）
4. 保存后随便改一行 commit 再 `git push`，或到 Actions → **Release FPK** → **Run workflow**
