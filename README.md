...
在 [Releases / latest](https://github.com/celesWuff/ktpwarp-server/releases/tag/latest) 下载 `ktpwarp-server.fpk`（或 `ktpwarp-server_all.fpk`），在飞牛应用中心 **手动安装** 即可。

`master` 每次 push 或 Actions 里手动 **Run workflow** 都会重新构建并覆盖 `latest` Release（无需打 tag）。

**Fork 仓库 Actions 不跑？** 先到 **Actions** 页点黄色条启用 workflow，再在 **Settings → Actions** 里开启 **Allow all actions** 与 **Read and write permissions**。
