...
在 [Releases](https://github.com/celesWuff/ktpwarp-server/releases) 页面下载与当前版本对应的 `ktpwarp-server.fpk`（例如 [v1.3.5-beta](https://github.com/celesWuff/ktpwarp-server/releases/tag/v1.3.5-beta)），在飞牛应用中心选择 **手动安装** 即可。

发布由 GitHub Actions 自动构建，并更新 **与 `package.json` 版本一致的 Release**（标签 `v1.3.5-beta` 等）：

- **每次 push 到 `master`**：重新打包 fpk，并更新 `v{version}` Release（版本号来自 `package.json`，**无需手动打 tag**）
- **升版**：修改 `package.json` 与 `fpk/manifest` 的 `version` 后 push，会生成新的 `v新版本` Release

也可在 Actions 页手动 **Run workflow** 触发一次构建。

**Fork 仓库 / 点 Run workflow 提示 `Failed to queue workflow run`？**

1. 打开仓库 **Actions** 页，若顶部有黄色条 **「Approve workflows in this fork」** 或 **「I understand… enable workflows」**，必须先点同意。
2. **Settings → Actions → General**：
   - **Actions permissions** → **Allow all actions and reusable workflows**
   - **Workflow permissions** → **Read and write permissions**
   - 若有 **Fork pull request workflows**，可一并打开（与本仓库 push 无关，但部分账号需开）
3. 保存后等 1～2 分钟，再 **Actions → Release FPK → Run workflow**，分支选 **master**。
4. 仍失败：用 `git commit --allow-empty -m "ci: trigger"` 后 `git push origin master`（**不必打 tag**）。

本地 `fpk/app/server/` 等构建目录已加入 `.gitignore`，不会进仓库。
